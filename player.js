...
function init() {
	if (navigator.userAgent.indexOf('CrMo') > 0
			|| navigator.userAgent.indexOf('iPhone OS') > 0
			|| navigator.userAgent.indexOf('Windows Phone OS') > 0
		...
		document.getElementById("OtherAD").innerHTML = '<h2>ADVARSEL!</h2>'
				...
				+ ' pr&oslash;v <a href="https://market.android.com/details?id=com.android.chrome">Chrome for Android</a>.</p>';
	} else {
		document.getElementById("OtherAD").innerHTML = '<h2>ADVARSEL!</h2>'
...
function PlayLQ() {
	hq = false;
	document.getElementById("player").innerHTML = '<audio controls autoplay loop>'
			+ '<source src="http://stream.sysrq.no:8000/01-nsr-mobile.ogg" type="audio/ogg">'
